---
title: Backyard Bird Tracking With AI
...
published: true
tag: "large project"
description: "How I consume data and get notified of birds detected by my BirdNET-Go instance."
amp:
  - video
---
...
- Intro
- I love birds, I always wanted to track them as a hobby to know what's around the house and when to look out for them
- Add my favorite examples so far of the videos. 
- Include the scripts used to generate the video files
- Mention the _realization_ I loved birds after playing Wingspan
- Emphasize that this is all for _hobby_ collection and I am still very early on in my birding interest
- Add stories of encourters (like the Osprey detections who then built a nest a week later)
- or the owls that hunt the Osprey babies
- The turkey detections and then confirmed by looking out around that time.
- The wind screen and how it cut down wind sound (usualy used on action cameras, but this one had a larger adhesive that I think will keep it on longer). Now I hear my neighbor's wind chimes, but I don't hear the wind. 
- Conclusion

- Post online
- Mention to the users in the thread about how this fixes the issue where the state is lost when the API is down (like when the machine first boots up)
  - https://community.home-assistant.io/t/displaying-birdnet-go-detections/713611/88
- Mentions there are other updates that I made to the cards and sensors.

Summary: It's a nice UI on top of BirdNET. BirdNET is a project that uses machine learning to identify bird species from audio recordings. It was developed by researchers at Cornell University and is available as an open-source project on GitHub.


## Intro

How many birds do you think visit your house every day? How many do you hear on your walks? Are there birds you remember hearing as a kid, but you couldn't remember how to identify them?

Back in 2023, some of my friends introduced me to the boardgame, [Wingspan](https://boardgamegeek.com/boardgame/266192/wingspan). Before then, I hadn't really thought too heavily about the variety of bird species around me. Sure, I thought it was cool to see an owl or a hawk fly over. Occasionally I would use my phone to ID a bird sound. But those thoughts never dwelled more than a few minutes. 

If you're unfamiliar, Wingspan is a resource management turn-based game. But it heavily involves interactions between different bird species and their traits. Each card contained information about birds I had never heard of before. Seriously, did you realize how many different species there are??

This awakened a new interest in me and I was hooked. After this introduction, I started to pay attention to the birds around me. I was using my phone to help identify birds more frequently. There was an itch I wanted to scratch though. What if I were able to detect birds around my house all of the time? 

{% include toc.html %}

I had actually been dreaming of implementing something like this for over a year. I had already rough-planned where I was going to run the electricity outside for a raspberry pi and how I was going to protect it.

Then I came across [this golden reply](https://www.reddit.com/r/homeassistant/comments/1jiqumn/comment/mjh7y6i/) by [bkw_17](https://www.reddit.com/user/bkw_17/) on a thread discussing lesser-known integrations with Home Assistant. Which brought to my attention the [BirdNET-Go](https://github.com/tphakala/birdnet-go) project. 

## BirdNET-Go

![BirdNET-Go main dashboard](/assets/img/2025/05/birdnet-go_main_dashboard.png)*My main dashboard on BirdNET-Go*

BirdNET-Go is a realtime BirdNET soundscape analyzer. But the best part, is they mentioned that it can use the microphones on your existing outdoor cameras! Now I no longer needed to plan how to run new electric and waterproof hardware outside to record bird sounds. I could just use the existing cameras I have.

_Huge_ shout out to [Tomi P. Hakala](https://github.com/tphakala) who built the BirdNET-Go software layer on top of the BirdNET project. His work in the codebase is amazing and without it I don't think I would have been able to get this project off the ground.

> Shameless plug; I've made a few [minor contributions](https://github.com/tphakala/birdnet-go/pulls?q=is%3Apr+is%3Amerged+author%3Aaav7fl) to the project because I think it's so awesome, and I think you should too.
...
![BirdNET-Go search dashboard](/assets/img/2025/05/birdnet-go_search_dashboard.png)*The search dashboard on BirdNET-Go showing some of my favorite locked detections*


- Camera setup
  - Windscreen [Insta360 X4 Mic Wind Muff](https://www.amazon.com/dp/B0CYGGTJHY)

![A white outdoor security camera with a black action-camera windscreen installed.](/assets/img/2025/05/camera_windscreen.jpg)*Outdoor security camera with an "Insta360 X4 Mic Wind Muff" installed over mic to reduce wind noise*
...
> A lot of this is dependent on using any release after [Nightly Build 20250427](https://github.com/tphakala/birdnet-go/releases/tag/nightly-20250427) which adds the analytics dashboard and my change to the BirdNET-Go API to return the species code in the daily summary API endpoint. This is used to generate the ebird links in the Home Assistant markdown card.

Here are a bunch of `command_line` sensors I created to pull data from the BirdNET-Go API and include the data in Home Assistant.

Each of the `command_line` sensors also has a matching `template` sensor. It is used to persist  data even when the BirdNET-Go api isn't available (like when the machine first boots up). **This is important!** 

In the event that the BirdNET-Go API is down, the `command_line` sensors will fail and the sensor will resolve to `unknown`. Normally this would not be a problem. But our automations are triggered by comparing the previous and current states. 

After the API comes back up, if the previous `command_line` sensor state was `unknown`, we will have no idea if the birds in the updated `state` are _new_ or not. 

We can work around these edge cases by persisting the data we last _knew_ about. This is done by creating a `template` sensor for each `command_line` sensor, and ignoring any `unknown` and `unavailable` states. The result is a `template` sensor that will always have the last known state of the `command_line` sensor.

Below, I choose to only use the `template` sensors in the automations, while continuing to use the `command_line` sensors for the dashboard cards. This way, the dashboard makes me immediately aware of any issues with the BirdNET-Go API, while the automations are still able to function as expected.

**⚠️ NOTE:** Since the `template` sensor ignores `unknown` and `unavailable` states (which is what happens when Home Assistant starts), you'll likely need to wait for the first successful _change_ of the `command_line` sensor data before the `template` sensor will update. If you're impatient, you can always force the `command_line` sensor `state` to change to _any other number_ in Home Assistant by using the [states tab](https://www.home-assistant.io/docs/tools/dev-tools/#states-tab) under the dev tools.

### Sensor for Daily Species Summary

This command line sensor fetches the daily summary of bird species detected by BirdNET-Go using the `/api/v2/analytics/species/daily` endpoint. It uses `jq` to ensure that the output is always a JSON array, even if the API returns null or an error string. The sensor's state is the count of unique species detected, and it also stores the full list of species as an attribute.
...
This command line sensor fetches the _overall_ species summary from BirdNET-Go using the `/api/v2/analytics/species/summary` endpoint. This means it gets the species summary for not just today, but since data was first recorded by BirdNET-Go. 

Just like the previous sensor, it uses `jq` to ensure that the output is always a JSON array, and has some basic error handling. The sensor's state is the count of unique species detected, and it stores the full list of species as an attribute.
...
### Sensor for Brand New Bird Detections

Last, this command line sensor fetches the list of newly detected bird species from BirdNET-Go using the `/api/v2/analytics/species/detections/new` endpoint. This is a list of birds that have been detected for the _very first time_ in the last 30 days.

<details markdown="block">

<summary>[Yaml] Home Assistant Sensor - Brand New Detections</summary>
...
- sensor:
    ...
    command: >
      curl -s 'http://YOUR_BIRDNET_ENDPOINT:YOUR_BIRDNET_PORT/api/v2/analytics/species/detections/new' |
      jq 'if type == "array" then {birds: .} end'
...
- trigger:
   - platform: state
     entity_id: sensor.birdnet_new_bird_detections_list
     not_to:
       - unknown
       - unavailable
  sensor:
   - name: BirdNET New Bird Detections List Persisted Data
     unique_id: birdnet_new_bird_detections_list_persisted_data
     state: '{{ trigger.to_state.state }}'
     attributes:
       birds: '{{ trigger.to_state.attributes.birds }}'
     unit_of_measurement: "species"
```
{% endraw %}
...
```
State attributes for sensor.birdnet_go_events exceed maximum size of * bytes. 
This can cause database performance issues; Attributes will not be stored
```

The easiest solution is to exclude the sensors from being recorded in the database. You can do this by adding the following to your `configuration.yaml` file:
...
recorder:
  exclude:
    entities:
      ...
      - sensor.birdnet_new_bird_detections_list_persisted_data
```

#### Faster Updates

Pro tip: In addition to the polling from the `command_line` sensors, you can also set up a simple automation that forces a new fetch from each of the endpoints by using the MQTT topic `birdnet` as a trigger. We don't really care about the payload, but instead we only care if _something_ changes. We treat this like a push trigger to then fetch from the separate `command_line` sensors on our own. This cuts down on the time it takes to get updates for the sensors to make them react as quickly as possible.
...
Below is the template sensor that listens to the MQTT topic and sets the payload to the state of the sensor.

<details markdown="block">

<summary>[Yaml] Home Assistant Sensor - BirdNET MQTT</summary>
...
Below is the automation portion. It listens to changes in the MQTT sensor, and on any change it triggers the `command_line` sensors to force an update. 
...
Below I've created a few `markdown` cards in Home Assistant to display the data from the BirdNET-Go sensors. I liked having all of the information in one place, and using markdown tables were the quickest way to get something out there.

Could there be improvements or optimizations to these Home Assistant dashboard cards below? Absolutely. But I won't let perfection be the enemy of good. I've been using them for months and I'm quite happy with them. 

### Card: Daily Species Summary With Sparklines

![Home Assistant dashboard showing today's BirdNET detections frequencies represented by a sparkline chart](/assets/img/2025/05/home_assistant_birdnet_sparkline_card.png)*Today's BirdNET detections and frequencies represented by a sparkline chart*

> Requires: [Sensor for Daily Species Summary](#sensor-for-daily-species-summary)

This is my favorite card and the one I find myself looking at the moent. This card closely matches what one would find on their BirdNET-Go dashboard. It shows today's detected speciees, their overall count, and a fun sparkline chart that represents the frequency of detections throughout the day.
...
Since there isn't an easy way to fit a full sparkline chart on mobile in the markdown card, I decided to "combine" all of the counts from the quietest hours into a single bar. 

- `Bar 1` = Combined counts of from 12:00 AM - 6:00 AM
- `Bar 2..13` = Hourly counts from 6:00 AM - 6:00 PM
- `Bar 14` = Combined counts of from 6:00 PM - 12:00 AM

I also included another idea from [Alexandre](https://community.home-assistant.io/t/displaying-birdnet-go-detections/713611/24) who suggested using the species code from BirdNET-Go and linking to the eBird website with it. 👏 This way you can quickly look up the species and see more information about it from your dashboard card. 
...
It's a fun problem to have, but sometimes it's difficult to scan through the daily detection summary to see what the most recent detections are. This card solves that problem by showing the most recent detections in a simple markdown table sorted by the last heard time in a human readable format.

Unlike the daily summary card, this card does not reset after midnight. Helpful for scanning for detected birds in the late evening from the prior day.
...
While the two summary cards have been great, I also found myself searching the list for birds I was interested in keeping an eye out for. To sovle this problem, I created _another_ card that takes in a case-_insensitive_ list of birds, and matches their common name with the list in the species summary sensor. 

This is helpful for birds that I might not see every day, but I want to know when they're back in the area without opening up the BirdNET-Go dashboard and flipping through the last few days.

It's a pretty manual process to add the birds to the list, but I change this so infrequently that it wasn't worth it to engineer any further.
...
![Home Assistant dashboard showing BirdNET's birds that have been detected for the first time](/assets/img/2025/05/home_assistant_birdnet_newest_birds.png)*A list of birds that have detected for the very first time (within the last 30 days)*

> Requires: [Sensor for Brand New Bird Detections](#sensor-for-brand-new-bird-detections)

Last, this fun card shows the birds that have been detected for the very first time in the last 30 days. This is a fun way to see what _new_ birds are in the area and if I should be on the lookout for them. 

_However_, as this relies on the new detections endpoint, once a bird makes this list and rolls off, it will never be on this list again. So it is a bit of a one-time use card, but still fun to track as I'm new to birding this year.
...
type: markdown
content: >-
  {% if has_value('sensor.birdnet_new_bird_detections_list') %}  
  {% set species_data = state_attr('sensor.birdnet_new_bird_detections_list', 'birds') %} 
  {% if species_data and species_data | count > 0 %}
    ...
      {%- set first_heard = strptime(time, '%Y-%m-%d') %}{{ name }} | &nbsp;&nbsp;{{count}} | &nbsp;&nbsp;&nbsp;{{ relative_time(first_heard) }} ago 
    {% endfor %}
  {% else %} 
    No recent bird data available. 
  {% endif %}
...
![A Pileated Woodpecker searching for food in a dead tree.](/assets/img/2025/05/pilwoo.jpg)*A Pileated Woodpecker pecking away at a dead tree out back.*

Here is where the fun begins. Below are my collections of notification that I've set up in Home Assistant based on the created sensors above. 

I've simplified some of the notifications from my set up today. But here are some ideas that you could could implement in your own setup:
...
On the Home Assistant side, I have an automation that is triggered by this webhook. It parses the incoming data payload and sends a notification to my phone.

By leveraging the custom species action in BirdNET-Go, I can get better real-time push notifications for specific bird species without having to constantly poll the API and check for changes. Although, I _am_ already doing that for some of the cards... 🤷‍♂️
...
Whenever you want to get notified for a bird detection, add a custom action under the `species settings` in BirdNET-Go that points the script below. Then pair it with the notification automation further down.

Make sure to include the `CommonName`, `Confidence`, `Time`, and `Source` parameters in the custom action (any order will do).

> Note: I've found that I need to restart my BirdNET-Go instance whenever I make a change to the species settings (like adding a custom action).
...
echo "Common Name:    $COMMON_NAME_VAL" >&2 # Should NOT have extra quotes here
...
There is a 15 minute cooldown between new notifications. _unless_ you press the reset button in the notification. Then it will reset the cooldown. Useful when you're actively trying to locate a bird.

All bird species that I mark as "watched" are grouped together at the moment (so if we get notified of a baltimore oriole, it won't notify us of a white-crowned sparrow until the cooldown is complete).

TODO: Add this automation to Home Assistant
TODO: Include timer information: https://www.home-assistant.io/integrations/timer/
TODO: Webhook setup is in BirdNET-Go
TODO: Home Assistant Automation. Note that trigger can only be hit once. So different labels need different automations (like when you have two people that want notifications at different times).

For the automation below, I also created a timer to prevent multiple notifications from being sent in a short period of time. This is useful if you have multiple cameras and they all detect the same bird at the same time, or you want to avoid getting bombarded while you're running outside to spot it.
...
- Add the `Bird Detection Timer Reset` automation below
    - Update the `timer.kyle_bird_notification_cooldown` to _your_ new timer `entity_id`

**Known downside:** Since this script is intended to be used for _any_ trigger by BirdNET-Go, it's possible that a bird you're _more_ interested in could get missed if the timer is on a cooldown triggered by a different bird a few minutes earlier. I've accepted this as a drawback because the tradeoff for having a single automation for multiple birds is worth it to me. The easiest (but by no means the best) way to work around this is creating a duplciate automation, duplicate script in BirdNET-Go, and a different webhook_id trigger.

However, I do include a helper `action` in the notification that lets the user to immediately `reset` the timer to allow more through. This has proven to be useful when a bird is moving _around_ my house and I'm not sure which side it's on.
...
actions:
  - variables:
      current_birds_list: >-
        {% set cbl = trigger.to_state.attributes.get('species_list') if
        ...
        {% endif %}
      previous_birds_list: >-
        {% set pbl = trigger.from_state.attributes.get('species_list') if
  ...
  - repeat:
      ...
      sequence:
        - condition: template
          value_template: >-
            {{ previous_birds_list | selectattr('species_code', 'eq',
            repeat.item.species_code) | list | length > 0 }}
        - condition: template
          value_template: >-
            {% set current_bird = repeat.item %}

            {% set previous_bird_data_list = previous_birds_list |
...
It will only send a notification for each bird species _once_. So it might be more useful to use my _other automation_ that sends a notification for any bird that hasn't been detected in the last * days (I use 60).
...
## Bonus: Generate Sharable Videos From Sound Detections

The audio download process from BirdNET-Go seems to be in flux at the moment.

If you download the bird sounds from audio player in the `Main Dashboard` or `Search Bar` at the top, it appears to download the _original_ audio file from the `/api/v1/media/audio` endpoint. That audio file will follow a naming scheme close to `<scientific_name>_<confidence>p_<timestamp>Z.<original_extension>`. 

However, if you use the newer `Search Dashboard` (from the left-hand side), you'll be presented with a much more advanced search interface. This is what I'm using at the moment.

I've included both scripts below, but I really only use the `Search Dashboard` one.
...
Filenames that match the `<scientific_name>_<confidence>p_<timestamp>Z.<original_extension>` pattern and use the `/api/v1/media/audio` endpoint.
...
Here are some of my favorite bird recordings I've captured so far.
...
Listen closely and you can hear the hummingbird's wings buzzing. 🐝

{% include video.html
  src="/assets/files/2025/05/2025-05-24_07-50-49_Ruby-throated_Hummingbird.mp4"
  poster="/assets/files/2025/05/2025-05-24_07-50-49_Ruby-throated_Hummingbird.png"
  controls=""
%}
...
![Libbie and Kyle spotting an Eastern Bluebird at the park](/assets/img/2025/05/birdwatching_1.jpg)*Libbie and Kyle spotting an Eastern Bluebird at the park*
